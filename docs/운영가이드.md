...
| Platform   | `/settings/infra`, `/vfs`, `/bundle/bucket`      | LLM env, VFS, 번들 저장소       |
...
에이전트·사용자·Wiki 파일을 브라우저에서 관리합니다. **Platform** → **VFS** → **Agent** 탭.
...
관리자 계정으로 로그인한 뒤 UI 캡처가 필요하면:

```bash
cd frontend
E2E_BASE_URL=https://agents.k8s-test \
