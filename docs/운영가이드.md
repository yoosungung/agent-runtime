...
# 특정 SHA로 배포 (short면 full로 정규화; GHCR은 full+short 태그)
IMAGE_TAG=<git-sha> make k8s-apply-dev

# short만 배포돼 ImagePullBackOff일 때 — 기존 full digest에 short 태그 추가
gh workflow run retag-ghcr-short-sha.yml -f full_sha=<40-char-sha>
...
| 증상               | 확인                                                         |
| ---------------- | ---------------------------------------------------------- |
| ImagePullBackOff | `registry-creds` secret, GHCR 태그(`IMAGE_TAG`) 일치. short SHA만 쓰면 full로 정규화하거나 `retag-ghcr-short-sha.yml` |
