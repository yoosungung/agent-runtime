...
|---|---|---|
| `kind` | ∈ `{"agent", "mcp"}` | 400 |
| `runtime_pool` | ∈ `{"agent:compiled_graph", "agent:adk", "agent:custom", "mcp:fastmcp", "mcp:mcp_sdk", "mcp:didim_rag", "mcp:t2sql"}` + `kind`와 prefix 일치 (`agent:*` ↔ `kind='agent'`) | 400 |
...
| `config`, `user_meta.config` | `dict`, 직렬화 ≤ `MAX_CONFIG_BYTES` (기본 64KB) | 400 / 413 |
