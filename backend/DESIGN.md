...
도메인: `path-graph==X.Y.Z` wheel ([GitHub Release](https://github.com/yoosungung/path-graph/releases)) — `path_graph.admin.*`, `path_graph.admin.lifecycle`. 로컬 sibling 개발: `uv.override.toml`로 editable override. Argo: `pipeline_argo.py`. ingest WF는 `batch_manifest_key`(S3)만 전달 — inline `batch_manifest`는 빈 문자열(Argo `resolve-manifest`가 inline을 우선하므로). 로컬 dev Argo 미연결 시 run → 503.
