...
- **Postgres 직결** — 여섯 테이블의 **쓰기 소유자**:
  ...
  - `chat_threads` (mutable, per-user Chat UI thread registry) — CRUD + soft delete
  ...
  - 마이그레이션: **`backend/migrations/0001_init.sql`** … **`0016_vfs_glob_indexes.sql`**. path-graph 연동 시 **`path_graph.migrations.iter_migration_sql()`** (`path-graph` 패키지). 적용은 `make db-migrate-all` 또는 `deploy/k8s/base/migration-job.yaml`의 `db-migrate-0016` Job.
...
도메인: `path-graph==X.Y.Z` wheel + **`backend.pipeline_domain`** → `path_graph.console` only. 로컬 sibling 개발: `uv.override.toml`. agent-base binding은 **`GET /internal/v1/pipeline/tenants/{tenant}/projects/{id}/binding`** (`X-Runtime-Caller: agent-pool`, Ingress 미노출).
