...
@pytest.mark.asyncio
async def test_delete_general_agent_allowed_without_hard_delete_flag(client: AsyncClient):
    from backend.app import app

    owner = await _insert_user(app.state, "delete-owner", tenant="acme")
    row = await _insert_source(
        app.state,
        {
            "name": "delete-me",
    ...
    gone = await client.get(f"/api/source-meta/{row.id}")
