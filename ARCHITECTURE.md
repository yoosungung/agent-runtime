...
- **`source_meta.visibility`**: 모든 `source_meta`(general·hermes·bundle·image, agent·mcp)의 invoke 접근 정책. `private`(생성자만) · `tenant`(동일 tenant) · `public`(로그인 사용자) · `allowlist`(`user_resource_access` 행 필요). auth `/verify`가 visibility + ACL을 병합해 `Principal.access[]`를 구성한다. `user_resource_access` grant/revoke API는 `visibility=allowlist`일 때만 grant 가능(revoke는 정리용으로 허용).
