...
describe("HermesModelField", () => {
  it("renders model source options", () => {
    renderField({});
    expect(screen.getByText("플랫폼 기본")).toBeInTheDocument();
    expect(screen.getByText("등록된 Preset")).toBeInTheDocument();
    expect(screen.getByText("모델 직접 지정")).toBeInTheDocument();
  });
  ...
  it("emits preset value when preset mode selected", () => {
    ...
    fireEvent.click(screen.getByLabelText(/등록된 Preset/i));
    expect(onChange).toHaveBeenCalledWith("preset:CLAUDE_SONNET");
  });

  it("emits explicit model value", () => {
    const onChange = renderField({});
    fireEvent.click(screen.getByLabelText(/모델 직접 지정/i));
  ...
  it("parses existing preset value", () => {
    ...
    expect(screen.getByRole("combobox")).toHaveValue("CLAUDE_SONNET");
