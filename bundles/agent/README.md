...
**런타임 계약** — [deploy/examples/agent-base/README.md](../../deploy/examples/agent-base/README.md) 와 동일:

- `def build_agent(cfg: dict, secrets: SecretResolver) -> NativeObj`
- LLM·tool API 키 → `source_meta.config`
- MCP 호출 → `MCP_GATEWAY_URL` + JWT (`agent_base.app.get_current_token()`)
...
- **MCP 툴** — `bundles/mcp/` 에 등록된 서버 (예: `mail-server`, `search-server`)
