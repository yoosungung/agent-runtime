...
| PG-3 | VFS `/wiki` mount — PG `vfs_wiki_files` + binding `wiki.vfs_mount` | [x] | 3.2.1 — [`test_wiki_vfs.sh`](deploy/examples/tests/e2e/test_wiki_vfs.sh) 클러스터 E2E |
...
- [x] **invoke SSE 슬림화** — agent-pool `compiled_graph` `run_stream`이 `on_chat_model_stream`만 `{"text":…}`로 emit. BFF·프런트는 슬림 포맷 pass-through + ADK/CUSTOM·레거시 LangGraph 이벤트 호환 유지.
