...
- **VFS**: `CompositeBackend` — `/` → `StateBackend`, `/agent/` → `vfs_agent_files`, `/user/` → `vfs_user_files`, 선택 project의 `wiki.vfs_mount` → `vfs_wiki_files` read-only backend (`VFS_DSN`). DSN: `VFS_DSN` only (no `PATH_GRAPH_DSN`).
